...
/**
  * Created by dengxiwen on 2017/6/6.
  */
object DBPool {

  private val max = 500                         //连接池连接总数
    ...
    AnyRef.synchronized({
      if(pool.isEmpty){
        ...
        for(i <- 1 to connectionNum){
          val conn = DriverManager.getConnection("jdbc:phoenix:"+properties.getProperty("zookeeper.host")+":" + properties.getProperty("zookeeper.port"))
          pool.push(conn)
          conNum +=  1
        }
      }
